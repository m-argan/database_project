...
{
    // Tests generated by Copilot (Claude Haiku 4.5), with minor changes made to fix errors
    // test_perform_alter_PK() and test_already_deleted() were added by me (Madeleine)
    private $conn;
    ...
    public function test_perform_alter_no_changes() {
        ...
        // check no changes
        $stmt = $this->conn->prepare("SELECT * FROM tutors WHERE tutor_id = ?");
        $stmt->bind_param("i", $first_tutor['tutor_id']);
        $stmt->execute();
        $result = $stmt->get_result();
        $updated_tutor = $result->fetch_assoc();
        $this->assertEquals($updated_tutor, $first_tutor, "No changes should have been made");
    ...
    public function test_perform_alter_PK() {
        ...
        $class_id = $first_class['class_number'];
        $_POST['class_number'] = $class_id;
        ...
        // Verify that the update DID NOT take place
        $stmt = $this->conn->prepare("SELECT * FROM classes WHERE class_number = ?");
        $stmt->bind_param("i", $class_id);
        $stmt->execute();
        $result = $stmt->get_result();
        $updated_class = $result->fetch_assoc();
        $this->assertNotEquals('DSC', $updated_class['subject_code'], "Cannot update on PK, no changes made");
    
    }

    // // Test which tries to alter already deleted entry
    // public function test_already_deleted(){
    //     $_SERVER["REQUEST_METHOD"] = "POST";
    //     $_GET['tablename'] = 'classes';
    //     // clear prev POST data
    //     unset($_POST);
    ...
    //     $first_class = $result->fetch_assoc();
    //     print("ddd");
    //     print(var_dump($first_class));
    //     // Delete first class
    //     print(var_dump($_POST));
    //     delete_records($results, $this->conn);

    //     // Try to change first class
    //     $class_id = $first_class['class_number'];
    //     $_POST['class_number'] = $class_id;
    //     $_POST['orig_class_number'] = $first_class['class_number'];
    //     $_POST['class_name'] = 'New name'; // Changed value
    //     $_POST['submit_btn'] = 'Submit';
        
    //     // should fail
    //     perform_alter($this->conn, $doExit = false);
    //     $stmt = $this->conn->prepare("SELECT * FROM classes WHERE class_number = ?");
    //     $stmt->bind_param("i", $first_class['class_number']);
    //     $stmt->execute();
    //     $result = $stmt->get_result();
    //     print(var_dump($result));
    //     $updated_class = $result->fetch_assoc();
    //     // made sure class name has stayed the same
    //     $this->assertEquals(first_class['class_name'], $updated_class['class_name'], 
